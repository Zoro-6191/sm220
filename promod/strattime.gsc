...
main()
{
	...
	for ( i = 0; i < players.size; i++ )
	{
		...
		if ( ( player.pers["team"] == "allies" || player.pers["team"] == "axis" ) && player.sessionstate == "playing" && isDefined( player.pers["class"] ) )
		{
			...
			player setMoveSpeedScale( 1.0 - 0.05 * int( isDefined( player.curClass ) && player.curClass == "assault" ) );
...
stratTimer()
{
	...
	matchStartTimer setTimer( game["PROMOD_STRATTIME"] + level.prematchPeriod );
