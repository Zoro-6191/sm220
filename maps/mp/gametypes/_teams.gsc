init(){ setPlayerModels();}
setPlayerModels()
{
	game["allies_model"] = [];game["axis_model"] = [];
	alliesCharSet = tableLookup( "mp/mapsTable.csv", 0, getDvar( "mapname" ), 1 );
	if ( isDefined( alliesCharSet ) && alliesCharSet != "" )game["allies_soldiertype"] = alliesCharSet;
	else if ( !isDefined( game["allies_soldiertype"] ) || !isDefined( game["allies"] ) )
	{ game["allies_soldiertype"] = "desert"; game["allies"] = "marines"; }
...
CountPlayers()
{
	players = level.players;allies = 0;axis = 0;
	...
	players["allies"] = allies;players["axis"] = axis;return players;
}
getJoinTeamPermissions( team )
{
	level.maxClients = getDvarInt( "sv_maxclients" );level.teamLimit = level.maxclients / 2;teamcount = 0;
	...
	if( teamCount < level.teamLimit )return true;
	else return false;
}