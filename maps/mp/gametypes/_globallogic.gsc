...
menuAxis()
{
	...
	if ( self.pers["team"] != "axis" )
	{
		...
		self setClientDvars( "cg_scoreboardheight", 435 );
	}
...
menuSpectator()
{
	...
	if(self.pers["team"] != "spectator")
	{
		...
		self setClientDvars( "cg_scoreboardheight", 500 );
	}
}
...
waitAndSpawnClient( timeAlreadyPassed )
{
	...
	if ( !maps\mp\gametypes\_tweakables::getTweakableValue( "player", "forcerespawn" ) && self.hasSpawned )
	{
...
Callback_StartGameType()
{
	...
	if (  !game["roundsplayed"] && !game["promod_first_readyup_done"] || ( game["SCORES_ATTACK"] > 0 || game["SCORES_DEFENCE"] > 0 ) )
...
Callback_PlayerKilled(eInflictor, attacker, iDamage, sMeansOfDeath, sWeapon, vDir, sHitLoc, psOffsetTime, deathAnimDuration)
{
	...
	if( isPlayer( attacker ) )
	{
		if ( attacker == self )
		{
			...
			else if ( sMeansOfDeath == "MOD_SUICIDE" && sHitLoc == "none" && self.throwingGrenade )
		...
		else
		{
			...
			{
				if ( sMeansOfDeath != "MOD_GRENADE" && !level.rdyup )
				{
