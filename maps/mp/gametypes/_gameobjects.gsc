	...
	for(i=0;i<entitytypes.size;i++){
		if(isdefined(entitytypes[i].script_gameobjectname)){
			...
			for(j=0;j<allowed.size;j++){
				for(k=0;k<gameobjectnames.size;k++)if(gameobjectnames[k]==allowed[j]){dodelete=false;break;}
				if(!dodelete)break;
			}
			if(dodelete)
...
init(){level.numGametypeReservedObjectives=0;precacheItem("briefcase_bomb_mp");precacheItem("briefcase_bomb_defuse_mp");precacheModel("prop_suitcase_bomb");level thread onPlayerConnect();}
onPlayerConnect(){level endon("game_ended");for(;;){level waittill("connecting",player);player thread onPlayerSpawned();player thread onDisconnect();}}
onPlayerSpawned(){
	self endon("disconnect");level endon("game_ended");
	for(;;){
		self waittill("spawned_player");
		self thread onDeath();self.touchTriggers=[];
		self.carryObject=undefined;self.claimTrigger=undefined;
		self.canPickupObject=true;self.killedInUse=undefined;
	}
}
onDeath(){
	level endon("game_ended");self waittill("death");
	if(isDefined(self.carryObject))self.carryObject thread setDropped();
}
onDisconnect(){
	level endon("game_ended");self waittill("disconnect");
	if(isDefined(self.carryObject))self.carryObject thread setDropped();
}
createCarryObject(ownerTeam,trigger,visuals,offset){
	...
	for(i=0;i<visuals.size;i++){visuals[i].baseOrigin=visuals[i].origin;visuals[i].baseAngles=visuals[i].angles;}
...
carryObjectUseThink(){level endon("game_ended");for(;;){self.trigger waittill("trigger",player);if(self.isResetting||!isAlive(player)||!self canInteractWith(player.pers["team"])||!player.canPickupObject||player.throwingGrenade||isDefined(self.carrier))continue;self setPickedUp(player);}}
carryObjectProxThink(){
	level endon("game_ended");
	for(;;){
		self.trigger waittill("trigger",player);if(self.isResetting||!isAlive(player)||!self canInteractWith(player.pers["team"])||!player.canPickupObject||isDefined(self.carrier))continue;
		if(isDefined(level.timeout_over)&&!level.timeout_over)return;self setPickedUp(player);
	}
}
pickupObjectDelay(origin){level endon("game_ended");self endon("death");self endon("disconnect");self.canPickupObject=false;for(;;){if(distanceSquared(self.origin,origin)>4096)break;wait 0.2;}self.canPickupObject=true;}
setPickedUp(player){
	player giveObject(self);self.carrier=player;
	...
	self updateCompassIcons();self updateWorldIcons();
}
updateCarryObjectOrigin(){
	level endon("game_ended");objPingDelay=5;
	for(;;)	{
		...
		}else{self.objPoints["allies"] maps\mp\gametypes\_objpoints::updateOrigin(self.curOrigin + self.offset3d);self.objPoints["axis"] maps\mp\gametypes\_objpoints::updateOrigin(self.curOrigin + self.offset3d);wait 0.05;}
	}
}
giveObject(object){self.carryObject=object;self thread trackCarrier();if(isDefined(object.carryIcon)){self.carryIcon=createIcon(object.carryIcon,50,50);self.carryIcon setPoint("CENTER","CENTER",223,167);}}
returnHome(){
	self.isResetting=true;self notify("reset");
	for(i=0;i<self.visuals.size;i++){self.visuals[i].origin=self.visuals[i].baseOrigin;self.visuals[i].angles=self.visuals[i].baseAngles;self.visuals[i] show();}
	self.trigger.origin=self.trigger.baseOrigin;self.curOrigin=self.trigger.origin;if(isDefined(self.onReset))self [[self.onReset]]();self clearCarrier();updateWorldIcons();updateCompassIcons();self.isResetting=false;
}
setDropped(){
	self.isResetting=true;self notify("dropped");
	if(isDefined(self.carrier)){trace=playerPhysicsTrace(self.carrier.origin +(0,0,20),self.carrier.origin -(0,0,2000),false,self.carrier.body);angleTrace=bulletTrace(self.carrier.origin +(0,0,20),self.carrier.origin -(0,0,2000),false,self.carrier.body);}
	else{trace=playerPhysicsTrace(self.safeOrigin +(0,0,20),self.safeOrigin -(0,0,20),false,undefined);angleTrace=bulletTrace(self.safeOrigin +(0,0,20),self.safeOrigin -(0,0,20),false,undefined);}
	droppingPlayer=self.carrier;
	if(isDefined(trace)){
		tempAngle=randomfloat(360);dropOrigin=trace;
		if(angleTrace["fraction"]<1&&distance(angleTrace["position"],trace)<10){forward=(cos(tempAngle),sin(tempAngle),0);forward=vectornormalize(forward - vector_scale(angleTrace["normal"],vectordot(forward,angleTrace["normal"])));dropAngles=vectortoangles(forward);}
		else dropAngles=(0,tempAngle,0);for(i=0;i<self.visuals.size;i++){self.visuals[i].origin=dropOrigin;self.visuals[i].angles=dropAngles;self.visuals[i] show();}
		self.trigger.origin=dropOrigin;self.curOrigin=self.trigger.origin;self thread pickupTimeout();}
	else{
		for(i=0;i<self.visuals.size;i++){self.visuals[i].origin=self.visuals[i].baseOrigin;self.visuals[i].angles=self.visuals[i].baseAngles;self.visuals[i] show();}
		self.trigger.origin=self.trigger.baseOrigin;self.curOrigin=self.trigger.baseOrigin;
	}
	if(isDefined(self.onDrop))self [[self.onDrop]](droppingPlayer);
	self clearCarrier();self updateCompassIcons();self updateWorldIcons();self.isResetting=false;
}
clearCarrier(){if(!isdefined(self.carrier))return;self.carrier takeObject(self);self.carrier.carryObject=undefined;self.carrier=undefined;}
pickupTimeout(){
	self endon("pickup_object");wait 0.05;mineTriggers=getEntArray("minefield","targetname");hurtTriggers=getEntArray("trigger_hurt","classname");
	for(i=0;i<mineTriggers.size;i++){if(!self.visuals[0] isTouching(mineTriggers[i]))continue;self returnHome();return;}
	for(i=0;i<hurtTriggers.size;i++){if(!self.visuals[0] isTouching(hurtTriggers[i]))continue;self returnHome();return;}
	if(isDefined(self.autoResetTime)){wait self.autoResetTime;if(!isDefined(self.carrier))self returnHome();}
}
takeObject(object){if(isDefined(self.carryIcon))self.carryIcon destroyElem();if(!isAlive(self))return;self.carryObject=undefined;self notify("drop_object");if(object.triggerType=="proximity")self thread pickupObjectDelay(object.trigger.origin);}
trackCarrier(){
	level endon("game_ended");self endon("disconnect");self endon("death");self endon("drop_object");
	while(isDefined(self.carryObject)&&isAlive(self)){
		if(self isOnGround()){trace=bulletTrace(self.origin +(0,0,20),self.origin -(0,0,20),false,undefined);if(trace["fraction"]<1)self.carryObject.safeOrigin=trace["position"];}wait 0.05;
	}
}
createUseObject(ownerTeam,trigger,visuals,offset){
	...
	}else{useObject.useRate=1;useObject thread useObjectUseThink();}return useObject;
}
setKeyObject(object){self.keyObject=object;}
useObjectUseThink(){
	if(isDefined(game["promod_do_readyup"])&&game["promod_do_readyup"])return;
	level endon("game_ended");
	for(;;)
	{
